...
void RansacTransformation::computeInliersAndError (
...
{ //output var
  ...
  if (! (mean_error > 0))
    ROS_DEBUG_STREAM("Transformation for mean error !> 0: " << transformation);
  if (! (mean_error > 0))
    ROS_DEBUG_STREAM(mean_error << " " << inliers_temp.size());

}

template<class InputIterator>
Eigen::Matrix4f RansacTransformation::getTransformFromMatches (const std::vector<Eigen::Vector4f,
    Eigen::aligned_allocator<Eigen::Vector4f> > & source_feature_locations_3d, const std::vector<
    Eigen::Vector4f, Eigen::aligned_allocator<Eigen::Vector4f> > & target_feature_locations_3d,
    InputIterator iter_begin, InputIterator iter_end, bool& valid, float max_dist_m)
{
  ...
  for (; iter_begin != iter_end; iter_begin++)
  {
    ...
    Eigen::Vector3f from (source_feature_locations_3d[this_id][0],
        source_feature_locations_3d[this_id][1], source_feature_locations_3d[this_id][2]);
    Eigen::Vector3f to (target_feature_locations_3d[earlier_id][0],
        target_feature_locations_3d[earlier_id][1], target_feature_locations_3d[earlier_id][2]);
    if (max_dist_m > 0)
...
bool RansacTransformation::getRelativeTransformationTo (const std::vector<Eigen::Vector4f,
    Eigen::aligned_allocator<Eigen::Vector4f> > & source_feature_locations_3d, const std::vector<
    Eigen::Vector4f, Eigen::aligned_allocator<Eigen::Vector4f> > & target_feature_locations_3d,
    ...
    float& rmse, std::vector<cv::DMatch>& matches, uint min_matches)
{
  ...
  paramReader.get ("max_dist_for_inliers", max_dist_m);
  paramReader.get ("ransac_iterations", ransac_iterations);
  ...
  for (int n_iter = 0; n_iter < ransac_iterations; n_iter++)
  {
    ...
    transformation = getTransformFromMatches (source_feature_locations_3d,
        target_feature_locations_3d, sample_matches.begin (), sample_matches.end (), valid,
        max_dist_m);
    if (!valid)
      continue; // valid is false iff the sampled points aren't inliers themself
    if (transformation != transformation)
      continue; //Contains NaN
    ...
    if (inlier_error > 1000)
      continue; //most possibly a false match in the samples
    ...
    transformation = getTransformFromMatches (source_feature_locations_3d,
        target_feature_locations_3d, matches.begin (), matches.end (), valid, max_dist_m); // compute new trafo from all inliers:
    if (transformation != transformation)
      continue; //Contains NaN
    computeInliersAndError (*initial_matches, transformation, source_feature_locations_3d,
        target_feature_locations_3d, inlier, new_inlier_error, dummy, max_dist_m * max_dist_m);
